...
describe('Category e2e test', () => {
  ...
  const categorySample = { description: 'Realigned Sports' };
  ...
  describe('new Category page', () => {
    ...
    it('should create an instance of Category', () => {
      cy.get(`[data-cy="description"]`).type('b Handmade').should('have.value', 'b Handmade');

      cy.get(`[data-cy="sortOrder"]`).type('46937').should('have.value', '46937');

      cy.get(`[data-cy="dateAdded"]`).type('2023-02-15').blur().should('have.value', '2023-02-15');

      cy.get(`[data-cy="dateModified"]`).type('2023-02-14').blur().should('have.value', '2023-02-14');

      cy.get(`[data-cy="status"]`).select('RESTRICTED');
